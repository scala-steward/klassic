...
impl NativeCodeGenerator {
    ...
    fn compile_binary(
        ...
        if matches!(op, BinaryOp::Equal | BinaryOp::NotEqual)
            && let NativeValue::RuntimeList { label } = lhs_value
        {
            let rhs_value = self.compile_expr(rhs)?;
            self.emit_runtime_list_equal_native(label, rhs_value, span)?;
            if op == BinaryOp::NotEqual {
                self.asm.cmp_reg_imm8(Reg::Rax, 0);
                self.asm.setcc_al(Condition::Equal);
                self.asm.movzx_rax_al();
            ...
            && self.expr_may_yield_runtime_list(rhs)
        {
            ...
            if let NativeValue::RuntimeList { label } = rhs_value {
                self.emit_runtime_list_equal_static_native(label, lhs_value, span)?;
                if op == BinaryOp::NotEqual {
                    self.asm.cmp_reg_imm8(Reg::Rax, 0);
                    self.asm.setcc_al(Condition::Equal);
                    self.asm.movzx_rax_al();
    ...
    fn emit_print_value_fragment(&mut self, fd: u64, value: NativeValue) {
        match value {
            ...
            NativeValue::RuntimeList { label } => {
                self.emit_print_runtime_list(fd, label);
            }
            NativeValue::StaticIntList { label, len } => {
    ...
    fn emit_append_native_string_to_runtime_buffer_dynamic(
        &mut self,
        output_base: Reg,
        output_offset: Reg,
        input: NativeStringRef,
        ...
        self.asm.push_reg(Reg::R8);
        self.asm.push_reg(Reg::Rdi);
        self.asm.mov_data_addr(Reg::Rsi, input.data);
        self.emit_load_native_string_len(Reg::Rdx, input.len);
        self.asm.mov_imm64(Reg::R8, 0);
        let loop_label = self.asm.create_text_label();
