...
- Current native codegen covers the first vertical slice: integer and boolean
  ...
  `String` / `List<String>` returns copied into call-site buffers, top-level and
  inline lambda calls with the same annotated parameter matching, and top-level
  lambda bindings lowered as static functions or inlined at call sites when they
