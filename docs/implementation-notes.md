...
static top-level values, including builtin aliases and static lambda values, by
rebinding those captures inside the emitted function body. Direct calls and value
...
while also allowing structurally equivalent branch-local lambda values and
canonical builtin function values to merge. If equivalent returned closures
...
`xs.map(f)` lowers into the same native path.
...
string-key maps and string sets also support runtime string membership queries
for `Map#containsKey` and `Set#contains`. Static
