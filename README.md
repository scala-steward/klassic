...
or annotated as `String` / `List<String>` also bind actual runtime `String` and
`List<String>` arguments directly, so small pass-through and string-literal
concatenation helpers do not require parameter annotations.
...
string concatenation, or `toString`, including bound interpolation/concatenation
strings and aggregate `toString` for lists, records, maps, and sets that contain
them.
...
Dynamic `if` expressions whose branches yield different native strings or
runtime line lists now copy the selected branch result into a fixed runtime
buffer, so the merged value remains printable, comparable, and usable in later
string/list helpers.
...
values into runtime strings for supported element, key, and value types, and
the same path is available through printing, interpolation, and string
concatenation.
List/map literal equality and `assertResult` can also compare evaluated runtime
native elements, keys, and values against compatible literal expectations.
...
Map literal `Map#get` / `.get` can also select runtime native values from
static or runtime keys, while preserving every entry's evaluation effects.
